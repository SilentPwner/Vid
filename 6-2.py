# الخلية 6 المعدلة بالكامل: اختيار الفيديو المناسب تلقائياً بناءً على وصف المستخدم
import cv2
import numpy as np
...
def apply_motion_based_on_description(static_image_path, parsed_description, output_path="final_output.mp4"):
    ...
    if action_video is None:
        action_video = "walking_action.mp4"  # فيديو افتراضي
    
    # اختيار فيديو الخلفية المناسب
    background_video = None
    # البحث في الكائنات والخلفية
    all_objects = parsed_description.get("objects", []) 
    background = parsed_description.get("background", "")
    
    for obj in all_objects + [background]:
    ...
    if background_video is None:
        background_video = "room_background.mp4"  # فيديو افتراضي
